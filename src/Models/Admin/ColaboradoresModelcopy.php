...
class ColaboradoresModelcopy {
    ...
    */
    ...
    /**
     * Obtiene los datos de un colaborador específico mediante su UUID.
     * @param string $public_id
     * @return array|false
     */
    public function obtenColaboradorporID($public_id) {
        try {
            $conn = Database::getConnection();

            // 1. Obtener datos principales y traducir UUID a ID interno
            $sql = "
            SELECT 
                iu.id_info,
                u.id_usuario,
                ...
                pv.public_id AS public_id_privada -- Necesario para el select del modal
            FROM priv_usuarios u
            JOIN priv_infousuario iu ON u.id_usuario = iu.id_usuario
            JOIN priv_roles r ON u.id_rol = r.id_rol
            JOIN priv_estatus e ON u.id_estatus = e.id_estatus
            ...
            // 2. Obtener contactos usando el ID interno (que ya recuperamos de forma segura)
            $id_usuario_interno = $colaborador['id_usuario'];
            $colaborador['correos'] = $this->getCorreosByUsuarioId($id_usuario_interno);
            $colaborador['telefonos'] = $this->getTelefonosByUsuarioId($id_usuario_interno);

            return $colaborador;

        } catch (\PDOException $e) {
            error_log("Error en funcion API: " . $e->getMessage());
            return false;
        }
    }

    //Funcion de crear colaboradores --- AUN EN PROCESO
